...
public class LocalOuterV3_1 {
    ...
    public static void main(String[] args) {
        ...
        // 추가로 내부 클래스에서 바깥 클래스를 참조하기 위해 사용하는 필드도 확인할 수 있다.
        //  - 내부 클래스는 당연히 바깥 클래스를 참조할 수 있어야 하기 때문에 필요한 것이다.

        // 이런 필드들은 자바가 내부에서 만들어 사용하는 필드들이다.

        // 정리
        // 지역 클래스는 인스턴스를 생성할 때 필요한 지역 변수를 캡처해서 인스턴스에 보관한다.
        // 그리고 지역 클래스와 인스턴스를 통해 지역 변수에 접근하면, 실제로는 지역 변수에 접근하는 것이 아니라 인스턴스에 있는 캡처한 캡처 변수에 접근한다.
    }
}